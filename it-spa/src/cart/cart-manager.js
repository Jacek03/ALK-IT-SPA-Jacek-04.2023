...
export const cartManager = {
	addItem(item, type) {
		...
		if (cart === null) {
			...
			content = JSON.parse(cart); // np. { 'Pokój unarny': { price: 170, quantity: 2 } } // if (content.hasOwnProperty(item.name))
	...
	getAllItems() {
		...
		if (cart === null) {
			...
			const content = JSON.parse(cart); // entry to jest [KLUCZ, WARTOSC]
	...
	getTotalPrice() {
		...
		if (cart === null) {
			...
			const content = JSON.parse(cart); // [{ price, quantity }, { price, quantity },  { price, quantity }, ...]
